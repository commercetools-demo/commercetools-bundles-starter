...
describe('status badge', () => {
  ...
  describe('get code', () => {
    it('when published and unmodified, should return published', () => {
      expect(getCode(true, false)).toEqual(PRODUCT_STATUS.PUBLISHED);
    });

    it('when published and modified, should return modified', () => {
      expect(getCode(true, true)).toEqual(PRODUCT_STATUS.MODIFIED);
    });

    it('when unpublished, should return unpublished', () => {
      expect(getCode(false, true)).toEqual(PRODUCT_STATUS.UNPUBLISHED);
    });
  });
});
